...
void Joint::PreStep(float inv_dt)
{
	...
	body1->velocity -= body1->invMass * accumulatedImpulse;
	body1->angularVelocity -= body1->invI * Cross(r1, accumulatedImpulse);

	body2->velocity += body2->invMass * accumulatedImpulse;
	body2->angularVelocity += body2->invI * Cross(r2, accumulatedImpulse);
}
