...
void gpc_add_contour(gpc_polygon *polygon, gpc_vertex_list *vertex, int hole)
{
	gpc_add_contour_parts(polygon, vertex->num_vertices, vertex->vertex, hole);
}

void gpc_add_contour_parts(gpc_polygon *polygon, int num_vertices,
										 gpc_vertex const * vertex, int hole)
{
	int             *extended_hole, c, v;
	gpc_vertex_list *extended_contour;

	/* Create an extended hole array */
	MALLOC(extended_hole, (polygon->num_contours + 1)
		* sizeof(int), "contour hole addition", int);

	/* Create an extended contour array */
	MALLOC(extended_contour, (polygon->num_contours + 1)
	...
	extended_contour[c].num_vertices= num_vertices;
	MALLOC(extended_contour[c].vertex, num_vertices
		* sizeof(gpc_vertex), "contour addition", gpc_vertex);
	for (v= 0; v < num_vertices; v++)
	{
		extended_contour[c].vertex[v]= vertex[v];
	}

	/* Dispose of the old contour */
	FREE(polygon->contour);
	FREE(polygon->hole);
...
{
	sb_tree       *sbtree= 0;
	it_node       *it= 0, *intersect=0;
	edge_node     *edge=0, *prev_edge=0, *next_edge=0, *succ_edge=0, *e0=0, *e1=0;
	edge_node     *aet= 0, *c_heap= 0, *s_heap= 0;
	lmt_node      *lmt= 0, *local_min=0;
	polygon_node  *out_poly= 0, *p=0, *q=0, *poly=0, *npoly=0, *cf= 0;
	vertex_node   *vtx=0, *nv=0;
	h_state        horiz[2];
	int            in[2], exists[2], parity[2]= {LEFT, LEFT};
	int            c=0, v=0, contributing=0, search=0, scanbeam= 0, sbt_entries= 0;
	int            vclass=0, bl=0, br=0, tl=0, tr=0;
	double        *sbt= 0, xb=0.0, px=0.0, yb=0.0, yt=0.0, dy=0.0, ix=0.0, iy=0.0;
											 ...
											 gpc_tristrip *result)
{
	...
	lmt_node      *lmt= 0, *local_min=0;
	polygon_node  *tlist= 0, *tn=0, *tnn=0, *p=0, *q=0;
	vertex_node   *lt=0, *ltn=0, *rt=0, *rtn=0;
	h_state        horiz[2];
	vertex_type    cft=NUL;
	int            in[2], exists[2], parity[2]= {LEFT, LEFT};
	int            s=0, v=0, contributing=0, search=0, scanbeam= 0, sbt_entries= 0;
	int            vclass=0, bl=0, br=0, tl=0, tr=0;
	double        *sbt= 0, xb=0.0, px=0.0, nx=0.0, yb=0.0, yt=0.0, dy=0.0, ix=0.0, iy=0.0;
