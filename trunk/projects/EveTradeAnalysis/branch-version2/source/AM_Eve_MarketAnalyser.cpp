...
namespace AutoMatic
{
	namespace Eve
	{
		...
		ProfitIndex MarketAnalyser::examineOrder(MarketOrder const & sellOrder_, MarketOrder const & buyOrder_)
		{
			...
			profitIndex._jumpsTotal = (_ignoreJumpsToStart ? 0 : profitIndex._jumpsToStart) + profitIndex._jumpsPerTrip*profitIndex._trips;
			...
			double jumpDiv = profitIndex._trips + 2.0*( (profitIndex._jumpsPerTrip*profitIndex._trips) + (_ignoreJumpsToStart ? 0 : profitIndex._jumpsToStart) );

			if(jumpDiv > 0.0)
			{
				profitIndex._index = profitIndex._profit / jumpDiv;
			}
			else
			{
				profitIndex._index = profitIndex._profit;
			}
